...
def main():
    ...
    character_name = input("Create your Character: ")
...
class Character:
    ...
    def use_item(self, item):
        if item == "potion":
            ...
            bonus_damage = random.randint(5, 15)  # Bonus damage between 5 and 15
            self.attack_points += bonus_damage
            print(f"{self.name} equips a sword, increasing attack power by {bonus_damage}. New attack power is {self.attack_points}.")
...
def battle(character, monster):
    while character.is_alive() and monster.is_alive():
        ...
        if user_choice == 'yes':
            item_choice = input("Which item would you like to use? (potion/sword): ").strip().lower()
            if item_choice in character.inventory:
                character.use_item(item_choice)
                character.inventory.remove(item_choice)  # Remove the used item from inventory
            else:
...
def main():
    ...
    character = Character(character_name, 100, 20, inventory=["potion", "sword"])  # Character starts with 100 HP, 20 attack points, and 2 items in inventory
    
    monster = Monster("Goblin", 80, 15)  # Monster starts with 80 HP and 15 attack points
